...
static int swz_mmap_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
    ...
    offset = (int64_t)(vmf->virtual_address - vma->vm_start);
#else
...
static int ioctl_do_wz_start(struct xdma_engine *engine, unsigned long arg)
{
    ...
    for(i=0; i<WZ_DMA_NOFBUFS; i++)
    {
        ...
        {
            //printk(KERN_INFO "START: freeing the descriptor %d %p\n",i,ext->desc[i]);
...
static int ioctl_do_wz_stop(struct xdma_engine *engine, unsigned long arg)
{
    ...
    for(i=0; i<WZ_DMA_NOFBUFS; i++)
    {
        if(ext->desc[i])
        {
            //printk(KERN_INFO "STOP: freeing the descriptor %d %p\n",i,ext->desc[i]);
            xdma_desc_free(engine->lro->pci_dev, 1, ext->desc[i], ext->desc_dma[i]);
            ext->desc[i]=NULL;
            ext->buf_dma_t[i]=0;
        }
    }
